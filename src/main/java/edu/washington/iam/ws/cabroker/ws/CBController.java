...
public class CBController {
  ...
  private CBSession processRequestInfo(
      HttpServletRequest request, HttpServletResponse response, boolean canLogin) {
  ...
  private void sendToLogin(
      HttpServletRequest request, HttpServletResponse response, String loginPath) {
  ...
  private ModelAndView loginPage(
    ...
    // remoteUser = "" This is where you hardcode a user for testing on non-shib installs
    ...
    if (remoteUser != null) {
      ...
      log.debug(
          "login: ck = ...;" + remoteUser + ";" + dbl + ";" + methodKey + ";" + modtime / 1000);
      String enc =
          IamCrypt.encode(
              Double.toString(modtime)
                  + ";"
                  + remoteUser
                  + ";"
                  + dbl
  ...
  public ModelAndView search(
      @RequestParam(value = "owner", required = false) String optOwn,
  ...
  public ModelAndView getCert(
    ...
    if ((cert.status == CBCertificate.CERT_STATUS_ISSUED
            || cert.status == CBCertificate.CERT_STATUS_EXPIRED)
        && cert.getExpires() != null) {
  ...
  public ModelAndView getPkcs7(
      @PathVariable("name") String certName,
      @RequestParam(value = "id", required = true) Integer id,
      HttpServletRequest request,
      HttpServletResponse response) {
  ...
  public ModelAndView putNewRequest(
    ...
    try {
      ...
      if (cert.dnC == null || !cert.dnC.equals("US"))
        throw new CBParseException("Country must be 'US'");
      if (cert.dnST == null || !(cert.dnST.equals("WA") || cert.dnST.equals("Washington")))
        throw new CBParseException("State must be 'WA' or 'Washington'");
      if (cert.dnO == null || !cert.dnO.equals("University of Washington"))
        throw new CBParseException("Organization must be 'University of Washington'");
      if (cert.lifetime > 12 && cert.ca == CBCertificate.IC_CA)
        throw new CBParseException("Lifetime must be 12 months.");
      if (cert.keySize > 0 && cert.keySize < 2048 && cert.ca == CBCertificate.IC_CA)
        throw new CBParseException("Key length must be 2048");
      verifyOwnership(cert, session.remoteUser);
  ...
  public ModelAndView putRenewRequest(
      @RequestParam(value = "id", required = true) Integer id,
      HttpServletResponse response,
      HttpServletRequest request) {
  ...
  public ModelAndView ajaxMember(
      @RequestParam(value = "dns", required = true) String paramDns,
  ...
  public ModelAndView ajaxOwnerAdd(
      @RequestParam(value = "id", required = true) Integer id,
      HttpServletResponse response,
      HttpServletRequest request) {
  ...
  public ModelAndView ajaxOwnerDel(
      @RequestParam(value = "id", required = true) Integer id,
      HttpServletResponse response,
      HttpServletRequest request) {
  ...
  private void verifyOwnership(CBCertificate cert, String user)
      throws NoPermissionException, DNSVerifyException {
    boolean wc = false;
    for (int i = 0; i < cert.names.size(); i++) {
      ...
      if (!cn.matches("([\\w]+[\\w\\-]*\\.)+[a-z]+"))
        throw new DNSVerifyException("CN or altName not valid.");
  ...
  private String formatError(String msg, Exception e) {
    return "<span align=\"center\"><h4>"
        + msg
        + "</h4><p>"
        + e.getMessage()
        + "<p>See <a href=\""
        + errorUrl
        + "\" target=\"_blank\">Certificate Service Errors</a> for details and solutions.</span>";
  }
