...
public class ICCertificateAuthority implements CertificateAuthority {
  ...
  private static String timeoutMessage =
      "No response from InCommon.  The service may be down temporarily. Please try again later.";
  ...
  // dev strings
  // private static String authDataFile =
  // "C:\\Users\\mattjm\\Documents\\spregworking\\incommoncert\\comodo.pw";
  // private static String orgAndSecretFile =
  // "C:\\Users\\mattjm\\Documents\\spregworking\\incommoncert\\comodo.os";
  private long authDataModified = 0;
  private long orgAndSecretModified = 0;

  // soap body and parts
  // authdata comes from file at init
  private String authDataTmpl =
      "<authData>"
  ...
  private static String getCustomerCertTypesBody =
      "<tns:getCustomerCertTypes xmlns:tns=\"http://ssl.ws.epki.comodo.com/\">AUTHDATA"
          + "</tns:getCustomerCertTypes>";

  private static String collectBody =
      "<tns:collect xmlns:tns=\"http://ssl.ws.epki.comodo.com/\">AUTHDATA"
          + "<id>ID</id><formatType>1</formatType></tns:collect>";

  private static String collectPKCS7 =
      "<tns:collect xmlns:tns=\"http://ssl.ws.epki.comodo.com/\">AUTHDATA"
          + "<id>ID</id><formatType>3</formatType></tns:collect>";

  private static String collectRenewedBody =
      "<tns:collectRenewed xmlns:tns=\"http://ssl.ws.epki.comodo.com/\">AUTHDATA"
          + "<renewId>ID</renewId><formatType>1</formatType></tns:collectRenewed>";

  private static String collectRenewedPKCS7 =
      "<tns:collectRenewed xmlns:tns=\"http://ssl.ws.epki.comodo.com/\">AUTHDATA"
          + "<renewId>ID</renewId><formatType>3</formatType></tns:collectRenewed>";

  private static String getCollectStatusBody =
      "<tns:getCollectStatus xmlns:tns=\"http://ssl.ws.epki.comodo.com/\">AUTHDATA"
          + "<id>ID</id></tns:getCollectStatus>";

  // SHA-2 types
  private String singleSSLType =
      "<certType><id>14042</id><name>InCommon SSL (SHA-2)</name>"
          + "<terms>1</terms><terms>1</terms></certType>";

  private String multiSSLType =
      "<certType><id>14044</id><name>InCommon Multi Domain SSL (SHA-2)</name>"
          + "<terms>1</terms><terms>1</terms></certType>";

  private String wildcardSSLType =
      "<certType><id>14837</id><name>InCommon Wildcard SSL Certificate (SHA-2)</name>"
          + "<terms>1</terms><terms>1</terms></certType>";
  ...
  private static String enrollBody =
      "<tns:enroll xmlns:tns=\"http://ssl.ws.epki.comodo.com/\">AUTHDATA"
          + "ORGANDSECRET"
          + "<csr>CSR</csr>"
  ...
  private static String renewBody =
      "<tns:renewById xmlns:tns=\"http://ssl.ws.epki.comodo.com/\">AUTHDATA"
  ...
  public int getCertificate(CBCertificate cert)
    ...
    if (cert.renewId != null
        && cert.renewId.length() > 0
    ...
    try {
      String body =
          collectBody
              .replaceFirst("AUTHDATA", authData)
              .replaceFirst("ID", String.valueOf(cert.caId));
      ...
      if (status == 2) {
        ...
        if (PEMHelper.parseCert(cert) == 0) {
          cert.status =
              CBCertificate
                  .CERT_STATUS_REQUESTED; // kind of an assumption, incommon does this when no cert
          // yet
          return 0;
        }
        Date now = new Date();
  ...
  public String getCertificatePKCS7(CBCertificate cert)
    ...
    if (cert.renewId != null && cert.renewId.length() > 0)
      return getRenewedCertificatePKCS7(cert); // always seems to work
    ...
    try {
      ...
      if (status == 0 && cert.status != CBCertificate.CERT_STATUS_RENEWING)
        throw new CertificateAuthorityException("certificate is being renewed");
  ...
  public int getRenewedCertificate(CBCertificate cert)
      throws CertificateAuthorityException, CBNotFoundException {
    log.debug(
        "get renewed cert for "
            + cert.caId
            + " renewId="
            + cert.renewId
            + " status="
    ...
    try {
      String body =
          collectRenewedBody
              .replaceFirst("AUTHDATA", authData)
              .replaceFirst("ID", String.valueOf(cert.renewId));
      ...
      if (status == 0) {
        ...
        if (PEMHelper.parseCert(cert) == 0) {
          cert.status =
              CBCertificate
                  .CERT_STATUS_RENEWING; // kind of an assumption, incommon does this when no cert
          // yet
          return 0;
        }
      }
  ...
  // the cert has already been gotten.  only update for errors
  public String getRenewedCertificatePKCS7(CBCertificate cert)
      throws CertificateAuthorityException, CBNotFoundException {
  ...
  public int requestCertificate(CBCertificate cert)
    ...
    if (cert.formNames.size() > 0) {
      ...
      for (int i = 0; i < cert.formNames.size(); i++)
        if (!cert.formNames.get(i).equals(cert.cn)) cert.names.add(cert.formNames.get(i));
    }
    ...
    if (cert.names.size() > 1) {
      body =
          body.replaceFirst("ALTNAME", "<subjAltNames>" + ans + "</subjAltNames>")
              .replaceFirst("CERTTYPE", multiSSLType);
    ...
    if (status < 0) {
      if (status == (-16)) throw new NoPermissionException("InCommon says no permission.");
        ...
        throw new NoPermissionException("InCommon says DCV expired or not validated.");
  ...
  public int renewCertificate(CBCertificate cert) throws CertificateAuthorityException {
    ...
    String body =
        renewBody
            .replaceFirst("AUTHDATA", authData)
            .replaceFirst("RENEWID", Integer.toString(cert.caId));
    int status = 0;
    try {
      Element resp = webClient.doSoapRequest(soapUrl, soapAction, body);
      if (resp == null) throw new CertificateAuthorityException("IO error to CA");
    ...
    if (99999 < status && status < Integer.MAX_VALUE)
      cert.status = CBCertificate.CERT_STATUS_RENEWING;
    return 0; // zero used to be the success code--if we got back an enrollment id above then it
    // worked.
  }
  ...
  private void sendNotices(CBCertificate cert) {
    ...
    for (int i = 0; i < cert.names.size(); i++)
      detail = detail + "  Alt name:     " + cert.names.get(i) + "\n";
    for (int i = 0; i < cert.history.size(); i++) {
      CBHistory h = cert.history.get(i);
      if (h.event == 1)
        detail = detail + "  Requested:    " + h.eventTime.toString() + " by " + h.netid + "\n";
      if (h.event == 2)
        detail = detail + "  Renewed:      " + h.eventTime.toString() + " by " + h.netid + "\n";
      if (h.event == 3)
        detail = detail + "  Revoked:      " + h.eventTime.toString() + " by " + h.netid + "\n";
    }
