...
public class UWCertificateAuthority implements CertificateAuthority {
  ...
  private static String timeoutMessage =
      "No response from the UWCA.  The service may be down temporarily. Please try again later.";
  ...
  public int getCertificate(CBCertificate cert)
    ...
    try {
      ...
      if (cert.status == CBCertificate.CERT_STATUS_EXPIRED)
        status = 7; // got cert but it is expired
  ...
  private void sendNotices(CBCertificate cert) {
    ...
    for (int i = 1; i < cert.names.size(); i++)
