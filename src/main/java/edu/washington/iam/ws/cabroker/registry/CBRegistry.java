...
public class CBRegistry {
  ...
  public List<CBCertificate> getCertificates(
      String owner, String name, boolean uwca, boolean icca) {
    return getCertificates(0, owner, name, uwca, icca);
  }

  public List<CBCertificate> getCertificates(
      int id, String owner, String name, boolean uwca, boolean icca) {
    ...
    try {
      ...
      String query =
          "select distinct certificate.id as id,dn,cn,ca,caid,revokepass,renewid,status,requested,issued,expires from certificate";
      if (id > 0) query = query + " where id=" + id;
      else if (name != null && name.length() > 0)
        query = query + ",name where certificate.id=name.id and name.name like '%" + name + "%' ";
      else if (owner != null && owner.length() > 0)
        query = query + ",owner where certificate.id=owner.id and owner.netid='" + owner + "' ";
  ...
  public List<CBCertificate> getWaitingCertificates(int ca) {
    ...
    try {
      ...
      stmt = conn.createStatement();
      String query =
          "select distinct certificate.id as id,dn,cn,ca,caid,status,renewid from certificate where ca="
              + String.valueOf(ca)
              + " and "
              + " (status="
              + String.valueOf(CBCertificate.CERT_STATUS_REQUESTED)
              + " or status="
              + String.valueOf(CBCertificate.CERT_STATUS_RENEWING)
              + ")";
  ...
  public int updateCertificate(CBCertificate cert) {
    ...
    try {
      ...
      if ((cert.status != old.status)
          || (cert.dn != null && !cert.dn.equals(old.dn))
      ...
      if (cert.status != old.status)
        stmt.executeUpdate("update certificate set status=" + cert.status + " where id=" + cert.id);
      ...
      if (cert.expires != null && !cert.expires.equals(old.expires)) {
        ...
        log.debug(
            "update certificate set expires='"
                + formatter.format(cert.expires)
                + "' where id="
                + cert.id);
        stmt.executeUpdate(
            "update certificate set expires='"
      ...
      if (cert.renewId != null && !cert.renewId.equals(old.renewId)) {
        stmt.executeUpdate(
            "update certificate set renewid='" + cert.renewId + "' where id=" + cert.id);
      }
  ...
  public void putCertificate(CBCertificate cert) throws CertificateManagerException {
    ...
    try {
      ...
      String cmd =
          "insert into certificate (dn,cn,ca,caid,status) values ('"
              + safeDn
              + "','"
              + cert.cn
              + "',"
  ...
  public void getHistory(CBCertificate cert) {
    ...
    try {
      ...
      stmt = conn.createStatement();
      String query =
          "select event,event_time,netid from history where id='"
              + cert.id
              + "' order by event_time";
  ...
  public int addHistory(CBCertificate cert, int event, Date et, String user) {
    try {
      ...
      stmt.executeUpdate(
          "insert into history (id,event, event_time, netid) values "
              + " ("
              + cert.id
              + ","
              + event
